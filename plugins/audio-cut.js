...
let handler = async (m, { conn, __dirname, args, usedPrefix, command, RafiButtonTeks }) => {
    try {
        let q = m.quoted ? m.quoted: m
        let mime = (q.msg || q).mimetype || ''
        if (!/audio/.test(mime)) return RafiButtonTeks(`Balas vn/audio yang ingin diubah dengan caption *${usedPrefix + command}*`)
        if (!args[0] || !args[1]) return RafiButtonTeks(`${usedPrefix + command} 00:00:30 00:00:30`)
