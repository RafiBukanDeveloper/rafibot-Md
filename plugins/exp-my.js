...
let handler = async (m, { conn, command }) => {
  ...
  const isMods = [conn.decodeJid(global.conn.user.id), ...global.owner.filter(([number, _, isDeveloper]) => number && isDeveloper).map(([number]) => number)].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
  const isOwner = m.fromMe || isMods || [conn.decodeJid(global.conn.user.id), ...global.owner.filter(([number, _, isDeveloper]) => number && !isDeveloper).map(([number]) => number)].map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
...
`.trim()
    conn.adReply(m.chat, caption, 'M Y  I N F O', '', flaImg.getRandom() + 'MY INFO', global.setting.namabot, m)
}
handler.help = ['my']
handler.tags = ['xp']
